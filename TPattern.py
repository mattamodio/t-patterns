...
class TPattern(object):
	...
	def getEventTypes(self):
		...
		for row in self.dbCursor.fetchall():
			num = len(self.observation_periods)
			self.observation_periods.append( (row[0],num) )
		#sys.exit()
		...
		num = 0
		for row in self.dbCursor.fetchall():
			event_type_name, count = row
			num+=1
			number_id = str(num)

			# limit infrequent event types
			if self.limit_infrequent_events and count<self.limit_infrequent_events: continue
		...
		# limit infrequent pairs
		if self.limit_infrequent_pairs:
			for pair in self.unprunedPairs():
				eventType1_name, eventType2_name = [str(e) for e in pair]

				# if one of the events is also pruned by limit_infrequent_events, prune it
				if eventType1_name not in self.mapCounterToEventtype or eventType2_name not in self.mapCounterToEventtype: continue

				eventType1, eventType2 = (EventType(name=eventType1_name), EventType(name=eventType2_name))
				eventType = EventType(first_event_type=eventType1, last_event_type=eventType2)
				self.candidatePatterns.append(eventType)

		# otherwise just add all candidates
		else:
	...
	def completenessCompetition(self, eventTypeList, outputdatafile):
		...
		output_dataset = [[0 for _ in xrange(len(completePatterns)+1)] for _ in xrange(len(self.observation_periods))]
		for row in xrange(len(output_dataset)):
			output_dataset[row][0] = self.observation_periods[row][0]
		...
		for i in xrange(len(completePatterns)):
			...
			for op in ops_with_pattern:
				for optuple in self.observation_periods:
					if op==optuple[0]:
						op=optuple[1]
						break
				output_dataset[op][i+1] = 1
	...
	def trimWithKL(self, observation_periods_with_event):
		...
		if self.kl_divergence(dist1, dist2) < self.kl_cutoff:
			...
			print "Did not trim event_type with KL divergence: {0}".format(self.kl_divergence(dist1, dist2))
			return False
